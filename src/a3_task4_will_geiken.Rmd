...
```
Takeaways:
The two plots are fairly similar in spread, although none of the control group reached a diameter of 6 cm. The annual group appears slightly more uniform in distribution across sizes, while the control group appears to have a stronger central tendency. However, it's possilbe that both groups are still normally distributed.
...
```
Takeaways: Both of the Q-Q plots follow more-or-less linear trends. There are a few outliers on the low end of both groups and maybe another at the high end of the control group, but the linear relationship still suggests that both groups have normally distributed samples.
...
kbl(urchin_table) %>% 
  kable_styling()
#create a new subset that will show statistics; group the data by treatment to separate the two groups; calculate the mean, median, standard deviation, and sample size for both species; and format the table
```
...
The means of both groups are relatively close (3.916667 cm for the annual treatment group and 3.725806 cm for the control group), with the annual treatment group having a higher mean diameter by 0.190861 cm. Considering that the urchins were measured to the nearest half centimeter, drawing any conclusions about how treatment affects size seems irresponsible. This is further supported by the standard deviations for each group (0.9662380 for the annual treatment and 0.8668646 for the control group), which show that the mean of each group is within one standard deviation of the other mean. So, while there was a difference in the mean diameter of both groups, we cannot responsibly conclude that it is because of the difference in treatment.

### Meaning of the p-value
Our p-value tells us that if these samples were selected from populations with the same mean, there is a 25.36% chance that these samples would have ended up having the difference in means that we found. That is well above zero, which means that it is entirely possible that these samples were selected from populations with the same mean, and we cannot reject our null hypothesis.
